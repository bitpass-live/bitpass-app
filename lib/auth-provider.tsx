...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('bitpass-user');
    if (savedUser) {
  ...
  const login = (userData: Partial<User>) => {
    ...
    setUser(fullUser);
    localStorage.setItem('bitpass-user', JSON.stringify(fullUser));
  };
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        setTimeout(() => {
          ...
          setUser(updatedUser);
          localStorage.setItem('bitpass-user', JSON.stringify(updatedUser));
          resolve();
        }, 500);
      } catch (error) {
        reject(error);
