...
export function useDraftEvent({ eventId, instanceId }: UseDraftEventParams) {
  ...
  const setDraftField = useCallback(<K extends keyof EventModel>(field: K, value: EventModel[K]) => {
  ...
  const addTicket = useCallback(
    async (data: CreateTicketInput) => {
      ...
      try {
        ...
        setDraftEvent((prev) => (prev ? { ...prev, ticketTypes: [...(prev.ticketTypes || []), newTicket] } : prev));
  ...
  const updateTicket = useCallback(
    async (ticketId: string, updates: Partial<TicketType>) => {
      if (!draftEvent) return;
